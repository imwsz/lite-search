...
if (showCommonUse == "undefined" || showCommonUse == undefined) {
	setStorage("showCommonUse", "website_close");
}
...
for (let item in jsonData.sideBar.content) {
	if (jsonData.sideBar.content[item].show) {
		sideBarTitle.innerHTML +=
			`
            <div id="${jsonData.sideBar.content[item].value}" class="title-icon" style="color:${jsonData.sideBar.content[item].color};border:1px dashed ${jsonData.sideBar.content[item].color};">
...
if (jsonData.copyright.show) {
	...
	copyrightContent = copyrightContent.replace("#author#", "My sharing");
...
function createHtml(inner) {
	...
	if (!inner.type) {
		sideBarHtml =
			`
            <div id="${inner.value}" class="setlist" style="border:1px solid ${inner.color};color:${inner.color};">
	...
	if (inner.type == "changebg" && inner.value == "changebg") {
		sideBarHtml =
			`
            <div id="${inner.value}" class="capsule" style="border:1px solid ${inner.color};">
...
function renderSetting(id, color, name) {
	return `
        <div id="${id}" class="capsule" style="border:1px solid ${color};">
...
function createWebsite() {
	...
	websiteData.forEach(item => {
		if (item.show) {
			...
			sideBarHtml +=
				`
                <a id='${item.value}AddCapsule' class="capsule" style="border:1px dashed ${item.color};">
...
function renderCapsule(data) {
	return `
        <a id='${data.name}' href='${data.url}' target="_blank" class="capsule" style="border:1px solid ${data.color};">
